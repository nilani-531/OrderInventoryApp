...
public class StoresService implements StoresServiceInterface{
    ...
    public StoresResponseDto createStore(StoresRequestDto dto) {
        ...
        String normalizedName = dto.getStoreName().replaceAll("\\s+", "").toLowerCase();

        if (repository.existsStoreByName(normalizedName)) {
    ...
    @Override
    public StoresResponseDto getStoreById(Integer storeId) {
    ...
    public StoresResponseDto getStoreByName(String storeName) {
        ...
        storeName = storeName.replaceAll("\\s+", "").toLowerCase();
    ...
    @Override
    public StoresResponseDto updateStore(Integer storeId, StoresRequestDto dto) {
        ...
        String normalizedNewName = dto.getStoreName().replaceAll("\\s+", "").toLowerCase();
        String normalizedExistingName = store.getStoreName().replaceAll("\\s+", "").toLowerCase();

        if (repository.existsStoreByName(normalizedNewName)
                && !normalizedExistingName.equals(normalizedNewName)) {
            throw new DuplicateResourceException("Store name already exists");
        }
